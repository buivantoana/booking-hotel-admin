...
import { updateAccounts } from "../../service/account";
...
function ActionMenu({ account, onToggleStatus, onViewDetail }) {
  ...
  return (
    <>
      ...
      >
        <MenuItem onClick={(e) => { handleClose(e); onViewDetail(account); }}>
          <DescriptionIcon fontSize="small" sx={{ mr: 1.5 }} />
          Chi tiết
        </MenuItem>

        {account.active ? (
          <MenuItem onClick={(e) => { handleClose(e); onToggleStatus(account, false); }}>
            <HighlightOff fontSize="small" sx={{ mr: 1.5 }} />
            Ngừng hợp tác
          </MenuItem>
        ) : (
          <MenuItem onClick={(e) => { handleClose(e); onToggleStatus(account, true); }}>
...
export default function ManagerAccountView({
  ...
  const handlePause = async () => {
    try {
      let result = await updateAccounts(selectedAccount.id, { active: selectedAccount.active == 1 ? false : true })
  ...
  return (
    <Box sx={{ p: { xs: 2, sm: 3, md: 4 } }}>
      ...
      <Paper sx={{ p: 3 }}>
        ...
        {/* Bộ lọc */}
        <Stack direction={{ xs: "column", md: "row" }} spacing={2} alignItems="end" mb={4}>
          <Box>
          ...
          <Stack direction="row" spacing={1}>
            <Button
              ...
              sx={{ bgcolor: "#98b720", borderRadius: "24px", minWidth: 120, height: 40 }}
            >
              Tìm kiếm
            </Button>
            <Button
              ...
              {loading ? (
                ...
                paginatedAccounts.map((account, index) => (
                  ...
                  >
                    ...
                    <TableCell sx={{ fontWeight: 500 }}>{account.email}</TableCell>
                    <TableCell>
                      <Chip
        ...
        <DialogContent>
          ...
          {selectedAccount && (
            <Box mt={2} display={"flex"} flexDirection={"column"} gap={"10px"}>
              <Typography><strong>Email:</strong> {selectedAccount.email}</Typography>
              <Typography><strong>Tên:</strong> {selectedAccount.name}</Typography>
              <Typography><strong>Vai trò:</strong> {selectedAccount.role}</Typography>
              <Typography><strong>Trạng thái:</strong> {selectedAccount.active ? "Đang hoạt động" : "Ngừng hợp tác"}</Typography>
              <Typography><strong>Số khách sạn:</strong> {selectedAccount.hotel_count || 0}</Typography>
            </Box>
          )}
        </DialogContent>
      </Dialog>
      ...
      <Dialog
        ...
        <DialogContent sx={{ pb: 3, padding: 1 }}>
          ...
          <Typography fontSize='14px' textAlign={"center"} color='#666'>
            {selectedAccount?.active ==0 ?`Sau khi tiếp tục hợp tác, khách sẽ nhìn thấy lại toàn bộ khách sạn của tài khoản này.
Chủ khách sạn có thể ngừng hợp tác lại trong tương lai.`:` Khách sẽ không nhìn thấy toàn bộ khách sạn của tài khoản này sau khi bạn ngừng hợp tác khách sạn. Chủ khách sạn có thể hợp tác lại trong tương lai.`}
        ...
        <DialogActions
          ...
          <Button
            ...
            sx={{
              ...
              {selectedAccount?.active == 0 ?"Xác nhận tiếp tục hợp tác" :"Xác nhận ngừng hợp tác"}
